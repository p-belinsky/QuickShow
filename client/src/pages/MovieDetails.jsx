...
const MovieDetails = () => {
  ...
  const handleFavorite = async () => {
    try {
      if(!user) return toast.error("You must be logged in to favorite a movie.");
      ...
      console.error(error);
      toast.error("An error occurred favoriting movie. Please try again.")
    }
  }
      ...
      <div className="flex flex-col md:flex-row gap-8 max-w-6xl mx-auto">
        ...
        <div className='relative flex flex-col gap-3'>
          ...
          <div className='flex items-center gap-4 flex-wrap mt-4'>
            ...
            <button onClick={handleFavorite} className='bg-gray-700 p-2.5 rounded-full transition cursor-pointer active:scale-95'><HeartIcon className={`w-5 h-5 ${favoriteMovies.find((movie) => movie._id === id) ? "text-primary fill-primary" : ""}`}/></button>
          </div>
        </div>
      </div>
